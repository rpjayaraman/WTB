...
module top_tb;
    ...
    initial begin
        ...
        @(posedge clk);
        while (!tif.awready || !tif.wready) @(posedge clk);
        tif.awvalid <= 1'b0;
        tif.wvalid  <= 1'b0;

        while (!tif.bvalid) @(posedge clk);
        $display("[45 ns] Received AXI4 WRITE Response BVALID OKAY");
        ...
        $display("[65 ns] Driving AXI4 READ Request: Addr=0x20");

        @(posedge clk);
        while (!tif.arready) @(posedge clk);
        tif.arvalid <= 1'b0;

        while (!tif.rvalid) @(posedge clk);
        $display("[75 ns] Received AXI4 READ Data: RDATA=0x%0h", tif.rdata);
